...
public class RegXlatorType
{
     ...
     */
    public RegXlatorType(int xlatorId,
            Class<? extends BaseRegXlator> xlatorClass,
            String xlatorDescription)
    {
    ...
    public BaseRegXlator createRegXlator()
    {
        ...
        {
            if (xlator.getLogger() != null)
            {
                xlator.getLogger().error(
                        "Unable to create the required RegXlator instance\n"
                                + e);
            }
            else
            {
                // fallback on system err
                System.err.println(
                        "Unable to create the required RegXlator instance\n"
                                + e);
            }
